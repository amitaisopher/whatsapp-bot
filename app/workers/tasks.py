...
from app.services.inventory_search import NLInventorySearchService, get_inventory_search_service
...
async def handle_incoming_whatsapp_message(ctx, customer_id: str, from_number: str, user_message: str):
    logger = ctx['logger']
    whatsapp_service: WhatsAppService = ctx['whatsapp_service']
    session: AsyncClient = ctx['session']
    inventory_search_service: NLInventorySearchService = get_inventory_search_service(customer_id, session)
    try:
        response = await inventory_search_service.process_message(user_message, from_number)
        if response:
            message_to_send: str = response.get("response", "No response available")
            await whatsapp_service.send_message(from_number, message_to_send)
            logger.info(f'Sent WhatsApp message to {from_number}')
            response = await whatsapp_service.send_message(from_number, message_to_send)
            logger.info(f'Responded to WhatsApp message from {from_number}')
            return response
        else:
            logger.warning(f'No response received for message from {from_number}')
            return None
...
class WorkerSettings:
    functions = [download_content, send_whatsapp_message, handle_incoming_whatsapp_message]
    on_startup = startup
